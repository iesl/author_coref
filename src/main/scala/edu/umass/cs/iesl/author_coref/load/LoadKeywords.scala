...
import edu.umass.cs.iesl.author_coref.db.KeywordsDB


object LoadKeywords {

  def load(file: File, codec: String):KeywordsDB = new KeywordsDB(loadLines(file.lines(codec).toIterable).toMap)
